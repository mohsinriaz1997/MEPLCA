...
using System;
using CA.API.Models;
using CA.API.Interfaces.MasterData;

namespace CA.API.Repository.AdministrationData
{
    public class ApprovalSetupRepo : IApprovalSetup
    {
        private WBCContext _DBContext;

        public ApprovalSetupRepo(WBCContext dBContext)
        {
        ...
        public async Task<List<MstApprovalSetup>> GetAllData()
        {
            ...
            try
            {
                await Task.Run(() =>
                {
                    oList = _DBContext.MstApprovalSetups
                    .Include(x => x.MstApprovalOriginators)
                    .Include(x => x.MstApprovalStages)
                    .Include(x => x.MstApprovalTerms)
                    .ToList();
                });
            }
            catch (Exception ex)
        ...
        public async Task<ApiResponseModel> Insert(MstApprovalSetup oMstApprovalSetup)
        {
            ...
            try
            {
                await Task.Run(() =>
                {
                    _DBContext.MstApprovalSetups.Add(oMstApprovalSetup);
        ...
        public async Task<List<MstForm>> GetApprovalDocs()
        {
            ...
            try
            {
                await Task.Run(() =>
                {
                    oList = _DBContext.MstForms.Where(x => x.FlgApprovalSetup == true).ToList();
                });
            }
            catch (Exception ex)
            {
