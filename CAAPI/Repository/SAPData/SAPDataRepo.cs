...
namespace CA.API.Repository.SAPData
{
...
";
        ...
        public List<SAPModels> GetBlanketAgreementDetailFromSAP(string ProductCode)
        {
            List<SAPModels> oList = new List<SAPModels>();
            ...
            try
            {
                ...
                using (SqlConnection conn = new SqlConnection(constr))
                {
                    string StrQuery = $@"SELECT o.bpcode,o.bpname,i.itemcode,i.itemname,t1.UnitPrice,t1.Currency
                                        FROM OOAT o
                                        inner join OAT1 t1 on o.AbsID=t1.AgrNo
                                        INNER JOIN OITM i on t1.ItemCode=i.ItemCode
                                        where o.Status='A' and i.ItemCode= '{ProductCode}'";
                    if (conn.State == System.Data.ConnectionState.Closed)
                    {
                        conn.Open();
                    }
                    ...
                    {
                        SAPModels oBOMItemDetailObjects = new SAPModels();
                        oBOMItemDetailObjects.bpCode = Convert.ToString(rdr["bpcode"]);
                        oBOMItemDetailObjects.bpName = Convert.ToString(rdr["bpname"]);
                        oBOMItemDetailObjects.bitemCode = Convert.ToString(rdr["itemcode"]);
                        oBOMItemDetailObjects.bitemName = Convert.ToString(rdr["itemname"]);
                        oBOMItemDetailObjects.BlUnitPrice = Convert.ToString(rdr["UnitPrice"]);
                        oBOMItemDetailObjects.BLCurrency = Convert.ToString(rdr["Currency"]);
                        oList.Add(oBOMItemDetailObjects);
                    }
                    conn.Close();
                }
