...
namespace CA.UI.Data.SAPData
{
    public class SAPDataService : ISAPData
    {
        ...
        public async Task<List<SAPModels>> GetBlanketAgreementFromSAP(string ProductCode)
        {
            try
            {
                List<SAPModels> oList = new List<SAPModels>();

                var request = new RestRequest($"SAPData/getBlanketAgreementDetailFromSAP?ProductCode={ProductCode}", Method.Get) { RequestFormat = DataFormat.Json };
